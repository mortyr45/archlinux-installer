...
test ! -f $workdir/OVMF_VARS.4m.fd && cp /usr/share/edk2-ovmf/x64/OVMF_VARS.4m.fd $workdir/OVMF_VARS.4m.fd
...
qemu-system-x86_64 \
  ...
  -drive if=pflash,format=raw,file=$workdir/OVMF_VARS.4m.fd,snapshot=on \
...
# -netdev user,id=net0,hostfwd=tcp::9090-:9090 # -nic user,net=192.168.200.0/24 #-nographic -serial tcp:127.0.0.1:1234,server=on,wait=off -monitor none # -device e1000,netdev=net0 #-device virtio-vga-gl \
